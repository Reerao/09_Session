...
let messageHistory = [];
  ...
  async function appendAssistantResponse(assistantMessage) {
    messageHistory.push(
        { 'role': 'system',
        'content': 'しりとりを続けてください。2文字以上の言葉で回答してください。その際、語尾に「ほげ」をつけて回答してください。'
        },
  ...
  //以下だと、storyareaにすでにあるテキストが上書きされてしまう
  // $('#ref').click(function() {
  //   console.log('クリック');
  //   $('#storyarea').text($('#chat-history').text());

  //   var ref = $('#chat-history').text();
  //   console.log(ref)

  // });

  //以下であれば、storyareaにすでにあるテキストに追加可能
  $('#ref').click(function() {
    console.log('クリック');
    ...
    $('#storyarea').text(combinedText); // 結合したテキストをstoryareaにセット
